...
namespace HoloToolkit.Unity
{
    ...
    public class TextToSpeechManager : MonoBehaviour
    {
        ...
        static private float[] ToUnityAudio(byte[] wavAudio, out int sampleCount, out int frequency)
        {
            ...
            // GetStringResult the frequency
            frequency = BytesToInt(wavAudio, 24);

            // GetStringResult past all the other sub chunks to get to the data subchunk:
            ...
            if (synthesizer != null)
            {
                try
                {
                    ...
                    Task.Run(async () =>
                    {
                        ...
                        // GetStringResult the size of the original stream
                        var size = speechStream.Size;

                        // Create buffer
                        byte[] buffer = new byte[(int)size];

                        // GetStringResult input stream and the size of the original stream
